...
use crate::{Hold,TimeLimit,CSVOUT,CsvDebug};
...
pub fn html2hold(path:&str,tl:&TimeLimit)->Result<(Hold,CsvDebug),Box<dyn Error>>{
    let now = Instant::now();
    println!("************************ START *************************");
    ...
    let csv_deb = CsvDebug{
        data_count,err_count,invalid_count,double_count,valid_count
    };
